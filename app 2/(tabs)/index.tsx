...
export default function HomeScreen() {
  ...
  const [inventoryScope, setInventoryScope] = useState<InventoryScope>(
    userProfile?.currentFamilyId ? 'family' : 'user'
  );
  ...
  useEffect(() => {
    if (userProfile?.currentFamilyId) {
      setInventoryScope('family');
      setFamilyLoading(true);
      const unsub = familyService.listenToFamilyMembers(userProfile.currentFamilyId, (members) => {
        setFamilyMembers(members);
        setFamilyLoading(false);
      });
      return () => unsub();
    } else {
      // No current family, default to user's personal scope
